  ...
  for (let i = 0; i < obras.length; i++) {
    strTexto += `
      <article class="col-12 col-sm-12 col-md-6 col-lg-6 col-xl-4   text-center border border-light  p-2">
        <img src="${obras[i].imagem}" alt="${obras[i].titulo}" class="img-fluid mb-2">
        <h2>${obras[i].titulo}</h2>
        <p>${obras[i].descricao}</p>
        <button  onclick="buscarObras(${obras[i].id})">Ver mais</button>
      </article>
    `;
  }
...
function buscarDetalhes() {
  ...
  if (obra) {
    telaDetalhes.innerHTML = `
      <div class="col-12 text-center " id="d">
        <img src="${obra.imagem}" alt="imagem" width="400px" heigth="400px" class="img-fluid mb-3 p-3">
        <h2>${obra.titulo}</h2>
        <p><strong>Descrição:<br></strong>${obra.descricao}</p>
        <p><strong>Ano de Criação:<br></strong> ${obra.data}</p>
